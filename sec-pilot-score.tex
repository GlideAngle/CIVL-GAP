...
\begin{align}
    ...
    \nonumber \\
    Cutoff &= ESSTime_{LastPilotAtESS} \nonumber \\
    \nonumber \\
    LC_p &= \sum_{i : tp_i \in TrackPointsInSS_p} k * TT(tp_i) * (togo_{i-1}^2 - togo_i^2) \\
...
\begin{align*}
    PLO &= PilotsLandedOut \\
    \forall p : p \not \in PLO : LC &= LC_p \\
    \forall p : p \in PLO \land TT(tp_{max}) < Cutoff: LC &= LC_{early} \\
    \forall p : p \in PLO \land TT(tp_{max}) \geq Cutoff: LC &= LC_{late}
\end{align*}
\begin{pg}
