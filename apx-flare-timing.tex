...
\begin{lstlisting}[caption={Distance achieved in flight and distance to the
landing spot, \texttt{*.mask-track.yaml}}]
nigh:
  - - Phil de Joux
    - togo:
        distance: 113.088594
...
We consider only pilots landing out in 100m chunks of the task. How many came
down in a chunk and how many are downward bound is shown. For each chunk with
a landing, we have its relative difficulty. Taking the sum of these under
a moving window we can award difficulty fractions to sorry pilots.

\begin{lstlisting}[caption={The difficulty of each section of the task with
a landing, \texttt{*.land-out.yaml}}]
minDistance: 5.0 km
bestDistance:
- 159.3 km
landout:
