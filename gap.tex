...
\begin{figure}[h]
    \centering
    \input{points-allocation-hg}
    \caption{Points allocation for hang gliding}
\end{figure}

\begin{figure}[h]
    \centering
    \input{points-allocation-pg}
...
\subsection{Line-Circle Intersections on a Plane}
...
\begin{enumerate}
    \item
        Line segment $\overleftrightarrow{AB}$ \textbf{crosses the circle in 1 point.}\\
The point \(R\) is the intersection of $\overleftrightarrow{AB}$
and the circle.
    \item
        Line segment $\overleftrightarrow{AB}$ \textbf{crosses the circle in 2 points.}\\
The point \(R\) is the intersection of the circle and the line segment
$\overleftrightarrow{AC'}$ where \(C'\) is the closest point on
$\overleftrightarrow{AB}$ to the center of the circle.
    \item
        Line segment $\overleftrightarrow{AB}$ \textbf{doesn’t cross the circle.}\\
The new point \(R\) is the intersection of the circle and line
$\overleftrightarrow{CK}$ where \(K\) is point on $\overleftrightarrow{AB}$ so
\(|AK| / |BK| = |AT| / |BT|\) (\(T\) is current best touching point). \(|AR|
+ |BR|\) is minimum when angles ARC and BRC are equal. Angles are equal when
\(|AK| / |BK| = |AR| / |BR|\).
\end{enumerate}
\end{enumerate}
